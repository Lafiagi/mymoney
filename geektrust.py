...
from services.data_conversion_services import DataConverter
from services.date_manipulation_services import DateManupulationServices
from services.data_sanitization_services import DataSanitizer
...
class PortfolioTracker:
    def __init__(self) -> None:
        try:
            ...
            if not os.path.isfile(file_path):
                print(f"Invalid File: to run file do python3 geektrust.py <file_path>")
                exit(1)
            ...
            print(f"Error: to run file do python3 geektrust.py <file_path>")
            exit(1)
    ...
    def execute_next_command(self, command_parameters: str) -> None:
        '''
            This method takes command parameters, and  
            execute the command appropriate command.
        '''
        ...
        if command == ALLOCATE:
            ...
            print(
                "Please enter a valid command. choices are: ALLOCATE, SIP, CHANGE, BALANCE, REBALANCE"
            )
            exit(1)

    def allocate_funds(self, allocation_amounts: list) -> None:
        '''
            This method takes a list of the initial 
            allocation amounts and set the respective 
            variables.
        '''
        try:
            (
            ...
            self.total_allocation = (
                self.equity_allocation + self.debt_allocation + self.gold_allocation
            )

        except ValueError:
            print(
                f"Please supply a valid command format is: ALLOCATE AMOUNT_EQUITY AMOUNT_DEBT AMOUNT_GOLD"
            )
            exit(1)
    ...
    def rebalance_investment(self) -> None:
        '''
            This rebalances the investment portfolio across,
            equity, debt and gold using the respective
            desired percentages.
        '''
        try:
            rebalance_month_data, rebalance_month = self.get_rebalance_month_data()
        ...
        rebalanced_debt = floor(self.desired_debt_percentage * total_portfolio_amount)
        rebalanced_gold = floor(self.desired_gold_percentage * total_portfolio_amount)
        ...
        print(
            f"{rebalanced_equity} {rebalanced_debt} {rebalanced_gold}"
        )
    ...
    def set_sip_values(self, monthly_sip_data: dict) -> bool:
        ...
        successful = True
        try:
            self.equity_sip, self.debt_sip, self.gold_sip = DataConverter.convert_to_int(
                monthly_sip_data
            )
            self.monthly_data[DateManupulationServices.get_get_all_months()[0]] = [
                self.equity_sip,
                self.debt_sip,
                self.gold_sip,
            ]
    ...
    def calculate_portfolio_after_change(self, month: str) -> None:
        '''
        This method calculates the porfolio after the change
        has taken effect. SIP is added on monthly basis starting
        from February. All data is a cummulation of the previous
        month portfolio and the respective sip, except for January
        which is the initial month.
        '''
        equity_sip = 0
        debt_sip = 0
        gold_sip = 0
        (
            ...
            previous_debt,
            previous_gold,
        ) = DateManupulationServices.get_previous_month_data(self.monthly_data, month)
        if month != DateManupulationServices.get_get_all_months()[1]:#if not January
        ...
        equity_change = floor(
            current_equity_amount + (current_equity_amount * self.equity_change_rate)
        )
    ...
    def calculate_desired_percentages(self) -> None:
        '''
        This method sets the desired percentages which is derived
        from the allocation ratio.
        '''
        self.desired_equity_percentage = self.equity_allocation / self.total_allocation
        self.desired_debt_percentage = self.debt_allocation / self.total_allocation
        self.desired_gold_percentage = self.gold_allocation / self.total_allocation
